...
点击工具栏「🧲」按钮，自动扫描并修复当前文档中所有失效的本地文件链接：

- **自动扫描**：检测文档内 `file:///` 链接，标记文件已不存在或路径变更的失效链接
- **三级搜索**：R1 原目录就近找 → R2 上级目录扩大搜 → R3 全盘深度搜，覆盖网盘挂载盘
- **指纹匹配**：通过文件大小 + 修改时间精确识别同名文件
- **自动 + 手动**：单结果自动修复，多结果候选选择
...
> 💡 小贴士：右键文件夹可以添加收藏，右键文件可以复制到 assets 并插入。深度搜索按 Enter 触发，支持多关键词空格分隔。失效链接修复点击工具栏 🧲 按钮即可一键扫描修复。
